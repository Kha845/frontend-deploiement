...
export default class OryxStore {
    BASE_URL = import.meta.env.VITE_API_URL + '/v1/dashboard-oryx';
    ...
    getDataLastOutputStock = async () => {
        try {
            ...
            console.log("HTTP Status:", response.status);
            const data = response.data;
            console.log('La derniere sortie de stock a oryx' , response.data)
            if (data.error) {
                this.rootStore.handleError(response.status, data.message, data);
                return Promise.reject(new Error(data.message));
            } else {
            ...
            console.error("Erreur de recuperation des donnees de sortie du stockeur oryx:", error);
            this.rootStore.handleError(error.response?.status || 500, error.message || "An error occurred", error);
        }
    }
    ...
    getDataLastStateStock = async () => {
        try {
            ...
            console.log("HTTP Status:", response.status);
            const data = response.data;
            console.log('La derniere sortie de stock au stockeur oryx' , response.data)
            if (data.error) {
                this.rootStore.handleError(response.status, data.message, data);
                return Promise.reject(new Error(data.message));
            } else {
            ...
            console.error("Erreur de recuperation des donnees de sortie de oryx:", error);
            this.rootStore.handleError(error.response?.status || 500, error.message || "An error occurred", error);
        }
    }
    ...
    getDataLastRecord = async () => {
        try {
            ...
            console.log("HTTP Status:", response.status);
            const data = response.data;
            console.log('Les donnees derniers etats d\'oryx', data)
            if (data.error) {
                this.rootStore.handleError(response.status, data.message, data);
                return Promise.reject(new Error(data.message));
            }
            ...
            const result = {
                ...
                derniereSortie: lastSortie
                    ? {
                          ...
                          uniteDeMesure: lastSortie.uniteDeMesure || null,
                      }
                    : null,
