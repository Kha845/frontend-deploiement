...
const DetailsEntrantIndustrielMoisORYX = () => {
  const { rootStore: { oryxStore } } = useStore();
  ...
  const initTable = async () => {
    try {
      ...
      const data = await oryxStore.getDataLastMonthInputStockIndustriel();
  ...
  return (
    ...
    >
      <Box display="flex" justifyContent="flex-start" alignItems="right" width='100%'>
      ...
      <Typography variant="h4" textAlign="center" gutterBottom color="success" marginLeft='12%'>
        TABLEAU DES ENTREES DE CARBURANTS EN FORMAT INDUSTRIEL DE ORYX POUR LE MOIS {month}
      </Typography>
      <Box
        sx={{
          display: 'flex',
...
export default DetailsEntrantIndustrielMoisORYX;
