...
//   const [dataFacturePayee, setdataFacturePayee] = useState<FactureData | null>(null);
  const [dataFacturePaiementEnCours, setdataFacturePaiementEnCours] = useState<FactureData[]>([]);
  const [filteredFactures, setFilteredFactures] = useState<FactureData[]>([]);
  const [startDate, setStartDate] = useState<string>('');
  const [endDate, setEndDate] = useState<string>('');
    const navigate = useNavigate();
    const { getDataPaiementEnCours}=  recapilatifStore;
    const facturePaiementEncours = async () => {
        try {
          ...
          setdataFacturePaiementEnCours(resData.details_facture);
    ...
    useEffect(() => {
      facturePaiementEncours();
    },[]);

    const handleFilter = () => {
        if (!startDate || !endDate) {
          setFilteredFactures(dataFacturePaiementEnCours); // Réinitialiser le filtre si une date est manquante
          return;
        }
        ...
        const filtered = dataFacturePaiementEnCours.filter((facture) => {
      ...
      const handleReset = () => {
        setStartDate('');
        setEndDate('');
        setFilteredFactures(dataFacturePaiementEnCours); // Réinitialiser le tableau à l'état initial
      };
    // Vérifier si les données sont disponibles avant de les afficher
    if (!dataFacturePaiementEnCours) {
    ...
    <Card sx={{ marginTop: 2, width: '100%' }}>
      ...
      <CardContent>
        <Typography variant="h4" gutterBottom color="success" textAlign="center">
          Liste des Factures qui sont en cours de paiement
        </Typography>
...
export default observer(FacturePaimentEnCours);
