...
const FacturePartiellementPayee = () => {
  ...
  const [endDate, setEndDate] = useState<string>('');
    ...
    const { getDataPayeePartiellement}=  recapilatifStore;
    const facturePayePartiellement = async () => {
        try {
          ...
          setdataFacturePayeePartiellement(resData.details_facture);
    ...
    useEffect(() => {
      facturePayePartiellement();
    },[]);

    const handleFilter = () => {
        if (!startDate || !endDate) {
          setFilteredFactures(dataFacturePayeePartiellement); // Réinitialiser le filtre si une date est manquante
          return;
        }
        ...
        const filtered = dataFacturePayeePartiellement.filter((facture) => {
      ...
      const handleReset = () => {
        setStartDate('');
        setEndDate('');
        setFilteredFactures(dataFacturePayeePartiellement); // Réinitialiser le tableau à l'état initial
      };
    // Vérifier si les données sont disponibles avant de les afficher
    if (!dataFacturePayeePartiellement) {
    ...
    <Card sx={{ marginTop: 2, width: '100%' }}>
      ...
      <CardContent>
        <Typography variant="h4" gutterBottom color="success" textAlign="center">
          Liste des Factures partiellement payees
        </Typography>
...
export default observer(FacturePartiellementPayee);
