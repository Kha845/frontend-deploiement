...
import { Card, CardContent, Typography, Grid, Box, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Button, TextField, IconButton, CircularProgress } from '@mui/material';
  ...
  const [endDate, setEndDate] = useState<string>('');
    ...
    const factureImPaye = async () => {
        try {
          ...
          setdataFactureImPayee(resData.details_facture);
    ...
    const handleFilter = () => {
        if (!startDate || !endDate) {
          setFilteredFactures(dataFactureImPayee); // Réinitialiser le filtre si une date est manquante
          return;
        }
        ...
        const filtered = dataFactureImPayee.filter((facture) => {
      ...
      const handleReset = () => {
        setStartDate('');
        setEndDate('');
        setFilteredFactures(dataFactureImPayee); // Réinitialiser le tableau à l'état initial
      };
   
    return (

    <Card sx={{ marginLeft: '98px', marginTop: 2, width: '142%' }}>
...
export default observer(FactureImpayee);
