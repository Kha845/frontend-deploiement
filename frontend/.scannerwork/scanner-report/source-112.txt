...
export default class SenStockStore {
    BASE_URL = import.meta.env.VITE_API_URL + '/v1/dashboards';
    ...
    getDataLastInputStockCommercial = async () => {
        try {
            ...
            console.log("HTTP Status:", response.status);
            const data = response.data;
            console.log('Les dernieres entre commercial d\'un' , response.data)
            if (data.error) {
                this.rootStore.handleError(response.status, data.message, data);
                return Promise.reject(new Error(data.message));
            } else {
    ...
    getDataLastOutputStock = async () => {
        try {
            ...
            console.error("Erreur de recuperation des donnees de sortie de senStock:", error);
            this.rootStore.handleError(error.response?.status || 500, error.message || "An error occurred", error);
        }
    }
            ...
            const result = {
                ...
                derniereInitiale: lastInitiale
                    ? {
                        ...
                        temperature: lastEntree.temperature_final || null,
                        uniteDeMesure: lastEntree.uniteDeMesure || null,
                      }
                    : null,
            };
